...
export default function PropostaComercial({ 
  ...
  propostaId,
  onPdfGerado,
  usarNomeCompleto = false // ✅ NOVO: usar nome_completo ao invés de Material/Classe/MPA
}) {
            ...
            <div ref={printRef} style={styles.container}>
              ...
              <div style={{ marginBottom: '10px' }}>
                ...
                <table style={styles.table}>
                  <thead>
                    <tr>
                      {/* ✅ CONDICIONAL: Nome completo ou Material/Classe/MPA */}
                      {usarNomeCompleto ? (
                        <th style={{...styles.th, width: '52%'}}>Produto</th>
                      ) : (
                        <>
                    ...
                    {produtos.map((p, i) => {
                      ...
                      return (
                        <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#f7fafc' }}>
                          {/* ✅ CONDICIONAL: Nome completo ou Material/Classe/MPA */}
                          {usarNomeCompleto ? (
                            <td style={styles.td}>{p.nome_completo || `${p.produto} ${p.classe} ${p.mpa}`.trim()}</td>
                          ) : (
                            <>
